...
class TestDataUtils(unittest.TestCase):
    ...
    def test_encode_data(self):
        ...
        inputId,attentionMask = data_utils.encode_data(self.dataset, self.tokenizer, self.max_seq_len)
        print(inputId)
        self.assertEqual(list(numpy.array(inputId).shape), [len(self.dataset), self.max_seq_len])
        self.assertEqual(inputId.dtype,torch.long)
        self.assertEqual(list(numpy.array(attentionMask).shape), [len(self.dataset), self.max_seq_len])
        self.assertEqual(attentionMask.dtype,torch.long)
